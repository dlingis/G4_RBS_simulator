...
{  
  ...
  if( verboseLevel >= 1 ){
    ...
    G4cout << std::setw( 5) << fTrack->GetCurrentStepNumber() << " "
        ...
        << std::setw(6) << G4BestUnit(fTrack->GetTrackLength(),"Length")
        << std::setw(10) << fTrack->GetVolume()->GetName()
        << std::setw(10) << G4BestUnit(fStep->GetDeltaTime(), "Time"); //mano prideta
      ...
      if (nbtrk) {
        ...
        for (size_t lp=0; lp<(*secondary).size(); lp++) {
          G4cout << "   "
                 ...
                 << "  time ="
                 << std::setw(12)
                 << G4BestUnit((*secondary)[lp]->GetGlobalTime(),"Time"); //GetGlobalTime() G4BestUnit((*secondary)[lp]->GetGlobalTime(),"Time");
          G4cout << G4endl;
        }
