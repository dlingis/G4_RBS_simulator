...
#ifndef ModAtimaLoss_h
#define ModAtimaLoss_h 1
...
class ModAtimaLoss : public G4VEmModel
{

public:

  explicit ModAtimaLoss(const G4ParticleDefinition* p = nullptr,
                                  const G4String& nam = "Atima modified");

  virtual ~ModAtimaLoss();
...
private:
  ...
  ModAtimaLoss & operator=(const  ModAtimaLoss &right) = delete;
  ModAtimaLoss(const  ModAtimaLoss&) = delete;
...
inline void ModAtimaLoss::SetParticle(const G4ParticleDefinition* p)
{
  if(particle != p) {
    particle = p;
...
inline void ModAtimaLoss::SetGenericIon(const G4ParticleDefinition* p)
{
...
inline G4double ModAtimaLoss::GetChargeSquareRatio() const
{
  return chargeSquare;
}
...
inline void ModAtimaLoss::SetChargeSquareRatio(G4double val)
{
  chargeSquare = val;
}
